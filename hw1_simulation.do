qui {
	...
	set obs 37706 //set num of observations
	}
	if c(N)==37706 { //simulation 
	    #delimit ; //change command delimiter to ;
		//column: create binary variable bnt, representing assignment to placebo
		//and treatment
		g bnt=rbinomial(1,.5);
		lab define Bnt 
		    0 "Placebo"  
		...
		tab bnt ; 
		//column: define binary female variable
		...
		tab female;
		//column: simulate race distribution from unif(0,100) and 
		//define race labels
		tempvar dem ;
		...
		tab race;
		//column: simulate and define ethnicity to latinx or other
		...
		//column: simulate and define countries (4 countries)
		tempvar country;
		...
		tab country;
		//column: simulate and define an age variable
		...
		//column: define bmi variable
		gen bmi=rbinomial(1, .351); tab bmi; 
		//simulate days since first dose and covid status
